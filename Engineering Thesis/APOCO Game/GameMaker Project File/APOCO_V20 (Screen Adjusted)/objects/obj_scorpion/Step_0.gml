...
if (global.is_paused) {
	...
	if (animation < 4) {
		...
		if (x_speed >= 0.1 && !injured) {
			sprite_index = spr_scorpion_right_walk;
		}
		if x_speed < -0.1 {
			sprite_index = spr_scorpion_left_walk;
		}
		...
		sprite_index = spr_scorpion_left_all;
		// Apply movement
		x += 0;
		y += 0;
		
		if (spawn_timer == 0) {
			var venom = instance_create_layer(x, y, 0, obj_venom);
			
			
			if (venomCount == 0) {
				spawn_timer = 60*0.2;
			}
			else if (venomCount == 1) {
				spawn_timer = 60*0.3;
			}
			venomCount += 1;
